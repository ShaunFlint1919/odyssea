...
function odyssea_lymphea_enqueue_scripts() {
    ...
    // Load asset manifest (generated by build script)
    $manifest_file = $theme_path . '/asset-manifest.php';
    if (file_exists($manifest_file)) {
        ...
        // Enqueue React app JavaScript as ES module
        if (!empty($assets['js'])) {
            wp_enqueue_script(
            ...
            add_filter('script_loader_tag', 'odyssea_lymphea_add_module_type', 10, 3);
        ...
        // Fallback: try to auto-detect assets
        $assets_path = $theme_path . '/assets';
        if (is_dir($assets_path)) {
            // Find CSS file
            $css_files = glob($assets_path . '/index-*.css');
            if (!empty($css_files)) {
            ...
            // Find JS file
            $js_files = glob($assets_path . '/index-*.js');
            if (!empty($js_files)) {
...
/**
 * Add type="module" to React app script tag
 */
function odyssea_lymphea_add_module_type($tag, $handle, $src) {
    if ('odyssea-lymphea-react-app' === $handle) {
